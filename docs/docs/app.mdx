...
| `native` | `boolean` | `false` | Let links navigate normally (server-side routing)   |
...
### Native Mode (Server-Side Routing)

```ts
App.start({ native: true });
```

In native mode, `w-nav` links work like regular HTML links - clicking them triggers a full page navigation to the server. The client-side router is disabled.

Best for:

- Traditional server-rendered applications (Rails, Django, Laravel, etc.)
- Multi-page apps with backend routing
- Progressive enhancement on existing sites
