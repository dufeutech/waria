...
<w-tabs orientation="vertical" value="settings">
  ...
  <w-slot panels>
    <div>
      ...
      <w-slot panel name="notifications"
        ><div>Notification preferences...</div></w-slot
      >
    </div>
  </w-slot>
</w-tabs>
...
The tab list gets `display: flex` with row/column direction set automatically by `[orientation]`. State is exposed via `aria-selected` on the tab buttons; inactive panels are hidden via `[hidden]`.
...
w-tabs w-slot[panel] > *[hidden] {
  /* already invisible */
}

/* Vertical layout — host becomes flex row, list flex column */
w-tabs[orientation="vertical"] w-slot[panel] > * {
  flex: 1;
}
```
...
| Selector                                        | Targets                     |
| ----------------------------------------------- | --------------------------- |
| `w-tabs w-slot[tab] > *[aria-selected="true"]`  | The active tab              |
| `w-tabs w-slot[tab] > *[aria-selected="false"]` | Inactive tabs               |
| `w-tabs w-slot[panel] > *:not([hidden])`        | The currently visible panel |
| `w-tabs[orientation="vertical"]`                | Vertical-layout tabs        |
